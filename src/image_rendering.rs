...
fn draw_text_on_image(
    ...
    text.lines().enumerate().for_each(|(i, line)| {
        let y_pos = i as i32 * line_height;
    ...
    for line in text.lines() {
        for word in line.split_whitespace() {
            let word_width: f32 = word
                .chars()
                .map(|c| font.glyph(c).scaled(font_size).h_metrics().advance_width)
                .sum();

            let needs_space = !current_line.is_empty();
            let total_width = current_width + (needs_space as u8 as f32 * space_width) + word_width;
